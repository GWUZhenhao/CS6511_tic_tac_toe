...
class Game():
    def __init__(self, caption):
        ...
        self.buttons.append(GiveupButton(self.screen, '', MAP_WIDTH + 30, BUTTON_HEIGHT + 45))
        self.is_play = False

        self.map = Map(CHESS_LEN, CHESS_LEN)
        self.player = MAP_ENTRY_TYPE.MAP_PLAYER_ONE
        self.action = None
        self.AI = tic_tac_toe(CHESS_LEN, 5)
        self.useAI = False
        self.winner = None
    ...
    def play(self):
        ...
        if self.is_play and not self.isOver():
            if self.useAI:
                x, y = self.AI.search(self.map.map, self.player)
                print('xxyy',x,y)
    ...
    def checkClick(self, x, y, isAI=False):
        ...
        if self.AI.check_win(self.map.map, self.player):
